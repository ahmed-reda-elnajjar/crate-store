...
export const BUILT_IN_MOTIONS = ["idle", "walk"];
const S = (x) => Math.sin(x);
const C = (x) => Math.cos(x);
const tri = (t, at, dur) => { const k = (t - at) / dur; return k < 0 || k > 1 ? 0 : 1 - Math.abs(2 * k - 1); };
/** Relaxed, slightly curled fingers (curl towards the palm, which faces the thigh), with a slow breathing drift. */
const fingers = (drift) => {
    const curl = (deg, k = 1) => (t, s) => ({ rz: -(s || 1) * (deg + 2.5 * k * drift(t)) });
    return {
...
const walk = () => {
    ...
    const ph = (t) => (2 * Math.PI * t) / T;
    const leg = (t, s) => { const p = ph(t) + (s > 0 ? Math.PI : 0); return { swing: A * S(p), knee: 5 + 40 * Math.max(0, C(p)) ** 1.4 }; };
    const arm = (t, s) => s * B * S(ph(t));
    return {
...
const idle = () => {
    const T = 6;
    const w = (t) => (2 * Math.PI * t) / T;
    const b = (t) => S((2 * Math.PI * t) / 3);
    return {
        duration: T, step: 0.05,
        bones: {
...
export function motionClip(root, name) {
    const def = DEFS[name]?.();
    if (!def)
        return null;
    root.updateMatrixWorld(true);
    const times = [];
    for (let t = 0; t <= def.duration + 1e-6; t += def.step)
    ...
    for (const [key, fn] of Object.entries(def.bones)) {
        for (const bone of bonesOf(root, key)) {
            ...
            if (moves)
                tracks.push(new THREE.VectorKeyframeTrack(`${bone.name}.position`, times, pv));
        }
    }
